...
test.describe('documentation screenshots', () => {
    ...
    test.use({ viewport: { width: 1100, height: 920 } });
    ...
    test('captures every wizard screen', async ({ page }) => {
        ...
        // Step 2 — official starter kits...
        await page.click('#nameNext');
        await capture(page, '02-starter-kits.png');

        // Step 2 — blank Blade setup with the UI framework presets...
        ...
        await capture(page, '03-ui-presets.png');

        // Step 3 — application options...
        await page.locator('.step:not(.hidden) [data-nav="next"]').click();
        await capture(page, '04-options.png');

        // Step 4 — version control...
        await page.locator('.step:not(.hidden) [data-nav="next"]').click();
        await capture(page, '05-version-control.png');

        // Step 5 — review with the equivalent CLI command...
        await page.locator('.step:not(.hidden) [data-nav="next"]').click();
        await capture(page, '06-review.png');

        // Success screen, rendered with representative content...
        await page.evaluate(() => {
            document.querySelectorAll('.step').forEach((el) => el.classList.add('hidden'));
            const progress = document.querySelector('[data-step="5"]')!;
        ...
        await capture(page, '07-success.png');
    });
});
